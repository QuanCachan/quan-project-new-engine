...
/**
 * <p>
 * An assertion accepts two resources (usually actual and expected results) and
 * test one against the other. If the test passes it returns nothing, otherwise
 * it *must* throw a {@link BinaryAssertionFailedException}. A
 * BinaryAssertionFailedException means a test failure, while any other
 * exception means a test error.</p>
 *
 * <p>
 * Remember : a BinaryAssertion must be annotated using @EngineComponent. In
 * this context the value of the annotation is the name of the category of the
 * assertion ("equals", "contains", "greater.than" etc), that will classify the
 * assertions regardless of the concrete implementation of Resource they
 * use.</p>
 *
 * <p>
 ...
 * @param <EXPECTED>
 */
public interface BinaryAssertion<ACTUAL extends Resource<ACTUAL>, EXPECTED extends Resource<EXPECTED>> {

    void setActualResult(ACTUAL actual);

    void setExpectedResult(EXPECTED expected);
     ...
     * {@link BinaryAssertionFailedException}.
     *
     * <p>
     * If some TECHNICAL anomaly in the INPUT resource is detected, this method
     * should throw {@link org.squashtest.ta.framework.exception.BadDataException}
     * </p>
     * <p>
     * If an assertion cannot work because it doesn't have the proper
     * configuration (see {@link #addConfiguration(Collection)}, it must throw an
     ...
     * </p>
     *
     * @throws BinaryAssertionFailedException
     */
    void test() throws BinaryAssertionFailedException;
}
